...
function Sidebar({ isOpen, set }) {
    return (
        <div onClick={(e) => e.stopPropagation()}
            className={`fixed h-screen z-20 top-0 w-[80%] max-w-sm bg-white p-3 px-5 transition-all duration-1000 ease-in-out flex flex-col ${isOpen ? 'left-0' : '-left-full'
                }`}>
            ...
            <nav className='flex-1 flex flex-col justify-end gap-4 pb-8'>
                <Link to={'#'} className='w-full text-center text-xl border rounded-lg p-4'>Sign In</Link>
                <Link to={'#'} className='w-full text-center text-xl border border-[#ff8906] rounded-lg p-4 bg-[#ff8906]'>Sign Up</Link>
            </nav>
        </div>
    )
}
